  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    output$msplot_gen <- dygraphs::renderDygraph({
      ...
      data <- app_state$ms() %>%
        ...
        plotly::select(
          ...
          dplyr::contains(input$ms_yaxis)
        ) %>%
        dplyr::mutate(dplyr::across(
          .cols = dplyr::contains('amu'),
    ...
    output$msplot <- plotly::renderPlotly({
      ...
      ms_plot <- app_state$ms() %>%
        dplyr::filter(
          dplyr::between(time_absolute_date_time, dates[1], dates[2]) &
    ...
    fit <- shiny::reactive({
      ...
      app_state$ms() %>%
        dplyr::filter(dplyr::between(
          time_absolute_date_time,
          lubridate::ymd_hms(range[1]),
          lubridate::ymd_hms(range[2])
        )) %>%
        dplyr::rename_with(
          .fn = ~ stringr::str_replace_all(.x, '_', ' '),
          .cols = dplyr::contains('_amu_')
