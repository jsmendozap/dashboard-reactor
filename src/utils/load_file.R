...
load_file <- function(file){
  readxl::read_excel(file) %>%
    ...
    dplyr::mutate(date_time = lubridate::dmy_hms(date_time),
                  ...
                  dplyr::across(.cols = c(2, 4, 6, 8),
                                .fns = ~ifelse(.x < 0, 0, .x))) %>%
    dplyr::rename(p_set = pressure_sp_history_out) %>%
    dplyr::group_by(event = assign_event(fic_110, fic_120, fic_130, fic_140, rswitch_val, p_set)) %>%
    ...
    dplyr::rowwise()%>%
    dplyr::mutate(name = stringr::str_glue("R1-{r1} R2-{r2} T-{temp} P-{p_set}")) %>%
    dplyr::ungroup()
}
