load_ms <- function(path, bd) {
  data <- bd %>%
    ...
    dplyr::left_join(
      bd %>% dplyr::select(date_time, tic_300_pv),
  ...
  data <- bd %>%
    ...
    ) %>%
    {
      dplyr::left_join(
        data,
        .,
        by = dplyr::join_by(dplyr::between(date_time, start, end))
      )
    }
  ...
  readr::read_delim(path, skip = 377, show_col_types = F) %>%
    ...
    dplyr::mutate(
      time_absolute_date_time = lubridate::mdy_hms(time_absolute_date_time)
    ) %>%
    dplyr::left_join(
      data,
      by = dplyr::join_by(closest(time_absolute_date_time >= date_time))
    ) %>%
    dplyr::rename_with(
      .fn = ~ substr(.x, 2, nchar(.)),
      .cols = dplyr::contains("_amu_")
    ) %>%
